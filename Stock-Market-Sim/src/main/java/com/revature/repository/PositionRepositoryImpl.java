...
public class PositionRepositoryImpl implements PositionRepository{
	...
	public Position getPosition(User user, Game game, String stockSymbol) {
		...
		try {
			return (Position) sessionFactory.getCurrentSession().createCriteria(Position.class).add(Restrictions.like("portfolio_id", port.getPortfolioId())).add("stock_symbol", stockSymbol)
					.list().get(0);
